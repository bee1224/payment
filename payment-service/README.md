...
**Code Verified / Test Verified：** 第一階段核心程式、Migration 與雙環境設定已存在。**Sandbox Deployment Complete / Callback Smoke Test Complete / Sandbox Verified：** 代收已完成真實 Sandbox 付款、Provider Notify、Ledger、Callback、retry 與重複 Notify 冪等驗收。**一般代付：** 尚未完成 Provider Sandbox 端對端驗證。**Manual Payout：** 尚未完成完整操作驗收。**Production Unverified：** 尚未部署或驗證，未宣告 Production Ready。
